...
if [[ ! -f "terraform/main.tf" ]]; then
    print_error "Please run this script from the aws-resource-manager directory"
    exit 1
fi
...
run_security_tests() {
    ...
    python3 security_group_remediation.py report --output test_security_report.json
    print_status "Report saved to test_security_report.json"
    echo ""
    
    print_info "Test 3: Testing SSH-specific detection..."
    python3 security_group_remediation.py find --ports 22
    echo ""
...
run_aws_cli_tests() {
    ...
    python3 aws_resource_manager.py ec2 --operation describe_security_groups
    echo ""
    ...
    python3 aws_resource_manager.py ec2 --operation describe_vpcs
    echo ""
}
